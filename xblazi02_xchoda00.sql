...
-- ukázka - výpočet ceny pro objednávku s ID 1
DECLARE 
  RESULT INT;
BEGIN
...
-- ukázka - přidání materiálu, který existuje (a je už i použit), a meteriálu, který ještě neexistuje
...
-- PŘÍSTUPOVÁ PRÁVA --
-- Role mechanik - může spravovat vše kromě mechaniků a přiřazování k termínům
CREATE ROLE ROLE_MECHANIK;
GRANT SELECT, INSERT, UPDATE ON ZAKAZNIK TO ROLE_MECHANIK;
GRANT SELECT, INSERT, UPDATE ON OBJEDNAVKA TO ROLE_MECHANIK;
GRANT SELECT, INSERT, UPDATE ON OPRAVA TO ROLE_MECHANIK;
GRANT SELECT, INSERT, UPDATE ON TERMIN TO ROLE_MECHANIK;
GRANT SELECT, INSERT, UPDATE ON MATERIAL TO ROLE_MECHANIK;
GRANT SELECT, INSERT, UPDATE ON VZTAH_MAT_OPR TO ROLE_MECHANIK;

-- MATERIALIZOVANÝ POHLED --
-- Seznam všech termínů, společně s informací o jejich opravě a objednávce
DROP MATERIALIZED VIEW OBJEDNAVKA_OPRAVA_TERMIN;

CREATE MATERIALIZED VIEW OBJEDNAVKA_OPRAVA_TERMIN
AS
SELECT REG_ZNACKA, ZNACKA_AUTA, MODEL, TYP_OPRAVY, POPIS_OPRAVY, MISTO, DATUM, CAS
FROM TERMIN TER
    JOIN OPRAVA OPR ON TER.REF_OPR = OPR.ID_OPR
    JOIN OBJEDNAVKA OBJ ON OPR.REF_OBJ = OBJ.ID_OBJ;

GRANT SELECT ON OBJEDNAVKA_OPRAVA_TERMIN TO ROLE_MECHANIK;

-- ukázka - všechny opravy a termíny auta s reg. značkou ABC-123
SELECT *
FROM OBJEDNAVKA_OPRAVA_TERMIN
WHERE REG_ZNACKA = 'ABC-123';

-- SELECT s CASE a WITH
